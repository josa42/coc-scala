...
const remove = async () => {
  ...
  await run(extensionsPath, 'yarn','unlink', 'coc-scala')

  const pkg = await readJSON(pkgPath)
  delete pkg.dependencies['coc-scala']
  await writeJSON(pkgPath, pkg)

  await run(extensionsPath, 'yarn','add', 'coc-scala')
}

const add = async () => {
  const pkg = await readJSON(pkgPath)
  pkg.dependencies['coc-scala'] = '*'
  await writeJSON(pkgPath, pkg)

  await run('.', 'yarn','link')
  await run(extensionsPath, 'yarn','link', 'coc-scala')
}
